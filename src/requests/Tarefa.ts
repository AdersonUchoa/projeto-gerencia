...
export const PostTarefa = async ({idCompromisso, descricao, status, dataConclusao}: {idCompromisso: number, descricao: string, status: number, dataConclusao: string}) => {
    return http.post(`/compromisso/${idCompromisso}/tarefa`, {descricao, status, dataConclusao});
}
